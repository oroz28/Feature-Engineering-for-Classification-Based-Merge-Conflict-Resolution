...
PROCESSED_DATA = Path("data/processed/preprocessed_merge_conflict_data_all.csv")
METHODS = ["chi2", "mutual_info", "rfe", "rf_importance"]
KS = [10, 20, 25, 30, 35, 40]


# 1. Load helpers
def load_metrics(model, language=None):
    """
    Aggregates all per-project metrics CSVs for a given model.
    """
    rows = []
    model_dir = RESULTS_DIR / model if language is None else RESULTS_DIR / language
    ...
    if not rows:
        raise FileNotFoundError(
            f"No metrics CSVs found in {model_dir} for model='{model}', language='{language}'. "
            "Run run_experiments.sh first."
        )
    return pd.concat(rows, ignore_index=True)


def load_feature_frequencies(model, language=None):
    """
...
def load_jaccard(model, language=None):
    """
...
def plot_performance_vs_k(summary, model, save, language=None):
    """
    ...
    plt.tight_layout()
    if save:
        path = RESULTS_DIR / model / f"{model}_performance_vs_k.png" if language is None else RESULTS_DIR / language / f"{model}_performance_vs_k.png"
...
def plot_feature_frequency(freqs, model, top_n, save, language=None):
    """ 
    ...
    plt.tight_layout()
    if save:
        path = RESULTS_DIR / model / f"{model}_feature_frequency.png" if language is None else RESULTS_DIR / language / f"{model}_feature_frequency.png"
        fig.savefig(path, dpi=150)
        print(f"Saved: {path}")
    plt.show()
...
# 4. Jaccard stability
def plot_jaccard_stability(jaccard_df, model, save, language=None):
    """
    ...
    plt.tight_layout()
    if save:
        path = RESULTS_DIR / model / f"{model}_jaccard_stability.png" if language is None else RESULTS_DIR / language / f"{model}_jaccard_stability.png"
        fig.savefig(path, dpi=150)
        print(f"Saved: {path}")
    plt.show()
...
def _load_feature_matrix(language=None):
    """ 
    ...
    if language:
        df = df[df['programming_language'] == language]
    return remove_leakage_features(df, language="all")


def compute_feature_correlations(language=None):
    """ 
    ...
    """
    X = _load_feature_matrix(language=language)
    X_numeric = X.select_dtypes(include=['number'])

    return X_numeric.corr(method='pearson')
...
def plot_correlation_heatmap(corr_matrix, model, save, language=None):
    """
    ...
    plt.tight_layout()
    if save:
        path = RESULTS_DIR / model / f"{model}_correlation_heatmap.png" if language is None else RESULTS_DIR / language / f"{model}_correlation_heatmap.png"
        fig.savefig(path, dpi=150)
        print(f"Saved: {path}")
    plt.show()
...
def plot_vif(vif_df, model, save, threshold=10.0, language=None):
    """
    ...
    plt.tight_layout()
    if save:
        path = RESULTS_DIR / model / f"{model}_vif.png" if language is None else RESULTS_DIR / language / f"{model}_vif.png"
        fig.savefig(path, dpi=150)
        print(f"Saved: {path}")
    plt.show()
...
def run_pca_variance_analysis(model, save, language=None):
    """
    ...
    plt.tight_layout()
    if save:
        path = RESULTS_DIR / model / f"{model}_pca_variance.png" if language is None else RESULTS_DIR / language / f"{model}_pca_variance.png"
        fig.savefig(path, dpi=150)
        print(f"Saved: {path}")
    plt.show()
...
def recommend_optimal_subset(summary, freqs, stable_threshold=0.8, corr_threshold=0.85, language=None):
    """
    ...
    try:
        corr = compute_feature_correlations(language=language)
...
def run_wilcoxon_tests(model, language=None):
    """
...
def main():
    ...
    print(f"=== Results Analysis — model: {model.upper()}, language: {language} ===\n")

    # Performance
    print("Loading metrics...")
    metrics_df = load_metrics(model, language=language)
    ...
    freqs = load_feature_frequencies(model, language=language)

    if freqs:
        plot_feature_frequency(
            freqs, model, args.top_n_features, args.save_plots, language=language)
    ...
    # Jaccard stability
    jaccard_df = load_jaccard(model, language=language)
    if not jaccard_df.empty:
        print("\n--- Jaccard Stability (mean across projects per method × k) ---")
        print(jaccard_df.to_string(index=False))
        plot_jaccard_stability(jaccard_df, model, args.save_plots, language=language)
    else:
        print(
    ...
    if PROCESSED_DATA.exists():
        ...
        plot_correlation_heatmap(corr, model, args.save_plots, language=language)
    else:
        print(f"Preprocessed data not found at {PROCESSED_DATA}.")

    # VIF multicollinearity analysis
    # print("\n--- VIF Multicollinearity Analysis ---")
    # vif_df = compute_vif(language=language)
    # if not vif_df.empty:
    #     severe = vif_df[vif_df['VIF'] > args.vif_threshold]
    #     print(f"Features with VIF > {args.vif_threshold} (severe multicollinearity): {len(severe)}")
    ...
    # PCA variance analysis
    if PROCESSED_DATA.exists():
        run_pca_variance_analysis(model, args.save_plots, language=language)
    ...
    run_wilcoxon_tests(model, language=language)
